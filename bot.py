...
# --- КОНФИГУРАЦИЯ ---
# Не храните токен в GitHub. Создайте переменную окружения BOT_TOKEN.
...
def send_keyboard(chat_id, text, keyboard):
    ...
    try:
        response = requests.post(url, data=data, timeout=10)
        if response.status_code != 200:
            print(f"Ошибка клавиатуры: {response.text}")
    except Exception as e:
        print(f"Ошибка: {e}")
...
def init_db():
    with closing(sqlite3.connect("economy_bot.db")) as conn:
        with conn:
            ...
            conn.execute(
                "INSERT OR IGNORE INTO event_shop "
                "(id, item_name, price) VALUES (1, 'Эксклюзивный скин', 100)"
            )
            conn.execute(
                "INSERT OR IGNORE INTO event_shop "
                "(id, item_name, price) VALUES (2, 'Обмен на монеты', 1)"
            )
            conn.execute(
                "INSERT OR IGNORE INTO custom_businesses "
                "(id, name, price, income_min, income_max, tax, level_req) "
                "VALUES (1, 'Кафе', 5000, 300, 600, 200, 3)"
            )
            conn.execute(
                "INSERT OR IGNORE INTO custom_businesses "
                "(id, name, price, income_min, income_max, tax, level_req) "
                "VALUES (2, 'Автосервис', 8000, 500, 1000, 300, 5)"
            )
...
def update_user(user_id, **kwargs):
    allowed = {
        "balance", "xp", "level", "work", "last_work_time",
        "house", "garage", "business", "warns", "last_tax_time",
        "event_points",
    }
    for key in kwargs:
        if key not in allowed:
            raise ValueError(f"Недопустимое поле users: {key}")
...
def handle_profile(chat_id, user_id):
    ...
    user = get_user(user_id)
    tax = (
        (200 if user["house"] else 0)
        + (150 if user["garage"] else 0)
        + (300 if user["business"] else 0)
    )
...
def handle_work(chat_id, user_id):
    ...
    user = get_user(user_id)
    last_work = (
        datetime.fromisoformat(user["last_work_time"])
        if user["last_work_time"] != "2000-01-01"
        else datetime.min
    )
    ...
    if datetime.now() - last_work < timedelta(minutes=10):
        ...
        send_message(
            chat_id,
            f"⏳ Отдых! Работа через {remaining.seconds // 60} мин.",
        )
        return

    work_data = None
    for work in WORKS.values():
        if work["level_req"] <= user["level"]:
            work_data = work
...
def handle_shop(chat_id, user_id):
    ...
    for item, price in SHOP_ITEMS.items():
        keyboard["inline_keyboard"].append(
            [{"text": f"{item} - {price}💰", "callback_data": f"buy_{item}"}]
        )
    keyboard["inline_keyboard"].append(
...
def handle_buybusiness(chat_id, user_id):
    ...
    if businesses:
        ...
        for business in businesses:
            keyboard["inline_keyboard"].append(
                [{
                    "text": f"{business[1]} - {business[2]}💰",
                    "callback_data": f"buybiz_{business[0]}",
                }]
            )
        keyboard["inline_keyboard"].append(
            [{"text": "❌ Закрыть", "callback_data": "close"}]
...
def handle_event(chat_id):
    ...
    if event:
        ...
        send_message(
            chat_id,
            f"🎯 *{event[1]}*\n\n{event[2]}\n"
            f"🎯 Задание: {task_ru} {event[4]} раз\n"
            f"🏆 Награда: {event[5]} очков",
        )
    else:
        ...
        for item in items:
            if item[1] == "Обмен на монеты":
                ...
                keyboard["inline_keyboard"].append(
                    [{
                        "text": f"🛒 {item[1]} - {item[2]}🎯",
                        "callback_data": f"eventbuy_{item[0]}",
                    }]
                )
        keyboard["inline_keyboard"].append(
            [{"text": "❌ Закрыть", "callback_data": "close"}]
...
def admin_stats(chat_id):
    with closing(sqlite3.connect("economy_bot.db")) as conn:
        ...
        balance = (
            conn.execute("SELECT SUM(balance) FROM users").fetchone()[0] or 0
        )
        points = (
            conn.execute("SELECT SUM(event_points) FROM users").fetchone()[0] or 0
        )
    send_message(
        chat_id,
        f"📊 *Статистика*\n\n"
        f"👥 Пользователей: {users}\n"
        f"💰 Общий баланс: {balance:,} B-coins\n"
...
def handle_admin_input(chat_id, user_id, text, state, event_data):
    if state == "admin_give":
        try:
            ...
            send_message(
                chat_id,
                f"✅ Выдано {amount} B-coins пользователю {uid}",
            )
            send_message(
                uid,
                f"💰 Администратор выдал вам {amount} B-coins!",
            )
        except Exception as e:
            send_message(
                chat_id,
                f"❌ Ошибка! Формат: ID сумма\n{str(e)}",
            )
        return None, None
    ...
    if state == "admin_promo":
        try:
            ...
            with closing(sqlite3.connect("economy_bot.db")) as conn:
                conn.execute(
                    "INSERT INTO promocodes (code, reward, uses_left) "
                    "VALUES (?, ?, 100)",
                    (code, reward),
                )
                conn.commit()
            ...
            send_message(
                chat_id,
                f"✅ Промокод {code} создан! Награда: {reward} B-coins",
            )
        except Exception as e:
            send_message(
                chat_id,
                f"❌ Ошибка! Формат: КОД СУММА\n{str(e)}",
            )
        return None, None

    if state == "admin_event_name":
        return "admin_event_desc", {"name": text}

    if state == "admin_event_desc":
        event_data = event_data or {}
        event_data["desc"] = text
        send_message(
            chat_id,
            "📝 Введите тип ивента (rob или steal):",
        )
        return "admin_event_type", event_data

    if state == "admin_event_type":
    ...
    if state == "admin_event_reward":
        try:
            ...
            with closing(sqlite3.connect("economy_bot.db")) as conn:
                conn.execute(
                    "INSERT INTO events "
                    "(name, description, task_type, task_count, reward_points, "
            ...
            send_message(
                chat_id,
                f"✅ Ивент создан!\n{event_data['name']}\n"
                f"Задание: {event_data['type']} {event_data['count']} раз\n"
                f"Награда: {text} очков",
            )
        except Exception as e:
...
def handle_callback(chat_id, message_id, user_id, data):
    ...
    if data.startswith("buybiz_"):
        try:
            biz_id = int(data.split("_", 1)[1])
        except ValueError:
            send_message(chat_id, "❌ Некорректный ID бизнеса")
            return

        with closing(sqlite3.connect("economy_bot.db")) as conn:
            biz = conn.execute(
                "SELECT * FROM custom_businesses WHERE id = ?",
                (biz_id,),
            ).fetchone()
        ...
        if biz:
            ...
            if user["business"]:
                ...
                send_message(
                    ...
                    f"🏢 *{biz[1]} куплен!*\n"
                    f"💰 -{biz[2]} B-coins\n"
                    f"💼 Доход: {biz[3]}-{biz[4]} B-coins/час\n"
                    f"⚠️ Налог: {biz[5]} B-coins/час",
                )
            else:
    ...
    if data.startswith("eventbuy_"):
        try:
            item_id = int(data.split("_", 1)[1])
        except ValueError:
            send_message(chat_id, "❌ Некорректный ID предмета")
            return

        with closing(sqlite3.connect("economy_bot.db")) as conn:
            item = conn.execute(
                "SELECT * FROM event_shop WHERE id = ?", (item_id,)
            ).fetchone()
        ...
        if item:
            ...
            if user["event_points"] >= item[2]:
                update_user(
                    user_id,
                    event_points=user["event_points"] - item[2],
                )
    ...
    if data.startswith("ad_"):
        ...
        if action == "give":
            user_states[user_id] = admin_give(chat_id)
            user_event_data[user_id] = None
        elif action == "broad":
            user_states[user_id] = admin_broadcast(chat_id)
            user_event_data[user_id] = None
        elif action == "promo":
            user_states[user_id] = admin_promo(chat_id)
            user_event_data[user_id] = None
        elif action == "event":
            user_states[user_id] = admin_event(chat_id)
                ...
                if "message" in update:
                    ...
                    if user_id in user_states:
                        ...
                        if state.startswith("admin_"):
                            new_state, new_data = handle_admin_input(
                                chat_id,
                                user_id,
                                text,
                                state,
                    ...
                    if text == "/start":
                        ...
                        if left:
                            send_message(
                                chat_id,
                                f"⏳ Следующее ограбление через "
                                f"{upgrade.fmt(left)}",
                            )
                        else:
                            upgrade.touch(user_id, "rob")
                            handle_rob(chat_id, user_id)
                            upgrade.event_action(user_id, "rob")
                    elif text == "/steal":
                        left = upgrade.cooldown(user_id, "steal", 1200)
                        if left:
                            send_message(
                                chat_id,
                                f"⏳ Следующий угон через "
                                f"{upgrade.fmt(left)}",
                            )
                        else:
                            upgrade.touch(user_id, "steal")
                            handle_steal(chat_id, user_id)
                            upgrade.event_action(user_id, "steal")
                    elif text == "/shop":
                    ...
                    # ===== РАСШИРЕНИЕ 2.0 =====
                        ...
                        upgrade.daily(chat_id, user_id)
                    elif text == "/bank":
                        upgrade.bank_menu(chat_id, user_id)
                        ...
                        try:
                            ...
                            send_message(
                                chat_id,
                                "❌ Формат: /withdraw сумма",
                            )
                    elif text.startswith("/pay "):
                        try:
                            parts = text.split()
                            upgrade.pay(chat_id, user_id, parts[1], parts[2])
                        except Exception:
                            send_message(
                                chat_id,
                                "❌ Формат: /pay ID сумма",
                        ...
                        upgrade.achievements(chat_id, user_id)
                    elif text == "/jobs":
                        upgrade.jobs(chat_id, user_id)
                        ...
                        try:
                            ...
                            send_message(
                                chat_id,
                                "❌ Формат: /sellcar номер",
                            )
                    elif text == "/realty":
                        upgrade.realty(chat_id, user_id)
                        ...
                        upgrade.business(chat_id, user_id)
                    elif text == "/bizcollect":
                        upgrade.bizcollect(chat_id, user_id)
                    elif text == "/bizupgrade":
                        upgrade.bizupgrade(chat_id, user_id)
                        ...
                        try:
                            ...
                            send_message(
                                chat_id,
                                "❌ Формат: /sell предмет количество",
                            )
